...
def run_simulation():
    ...
    system = SimulatedCrescendoSystem(
        ...
        check_interval=1.0,
        presence_timeout=5.0  # Shorter timeout for testing
    )
    ...
    try:
        ...
        timeout = system.presence_timeout + 1
        logger.info(f"Waiting {timeout} seconds for presence timeout...")
        time.sleep(timeout)
        
        # Verify that music stopped
        if not system.audio_player.is_playing():
            logger.info("✓ System correctly stopped playing music after timeout")
        else:
            logger.error("✗ System failed to stop playing music after timeout")
